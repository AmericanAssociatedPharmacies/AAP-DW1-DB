...
/****** Object: SqlProcedure [dbo].[DeleteObject] Script Date: 12/21/2020 8:48:05 AM ******/
SET ANSI_NULLS ON
GO
...
WHERE
    (R.Path = @Path OR R.Path LIKE @Prefix ESCAPE '*')
        ...
        UPDATE [aapssrs01_ReportServerTempDB].dbo.TempDataSources
           SET
              Flags = Flags & 0x7FFFFFFD, -- broken link
              Link = NULL
        FROM
           [Catalog] AS C
           INNER JOIN [aapssrs01_ReportServerTempDB].dbo.TempDataSources AS DS ON C.[ItemID] = DS.Link
        WHERE
           (C.Path = @Path OR C.Path LIKE @Prefix ESCAPE '*')
...
UPDATE [aapssrs01_ReportServerTempDB].dbo.TempDataSets
   SET
      [LinkID] = NULL
FROM
   [Catalog] AS C
   INNER JOIN [aapssrs01_ReportServerTempDB].dbo.TempDataSets AS DS ON C.[ItemID] = DS.[LinkID]
WHERE
   (C.Path = @Path OR C.Path LIKE @Prefix ESCAPE '*')
...
BEGIN
        ...
        SET PermanentRefcount = PermanentRefcount - 1
        FROM
           [aapssrs01_ReportServerTempDB].dbo.TempCatalog AS R WITH (XLOCK)
           INNER JOIN [aapssrs01_ReportServerTempDB].dbo.SnapshotData AS SD ON R.Intermediate = SD.SnapshotDataID
        WHERE
           R.ContextPath = @Path
        ...
        -- Clean all data sources
        DELETE [aapssrs01_ReportServerTempDB].dbo.TempDataSources
        FROM
            [aapssrs01_ReportServerTempDB].dbo.TempCatalog AS R
            INNER JOIN [aapssrs01_ReportServerTempDB].dbo.TempDataSources AS DS ON R.TempCatalogID = DS.ItemID
        WHERE
            R.ContextPath = @Path
            AND R.EditSessionID = @EditSessionID
            AND R.OwnerID = @OwnerID
        ...
        DELETE [aapssrs01_ReportServerTempDB].dbo.TempDataSets
        FROM
            [aapssrs01_ReportServerTempDB].dbo.TempCatalog AS R
            INNER JOIN [aapssrs01_ReportServerTempDB].dbo.TempDataSets AS DS ON R.TempCatalogID = DS.ItemID
        WHERE
            R.ContextPath = @Path
            AND R.EditSessionID = @EditSessionID
            AND R.OwnerID = @OwnerID
        ...
        FROM
           [aapssrs01_ReportServerTempDB].dbo.SnapshotData AS SN
           INNER JOIN [aapssrs01_ReportServerTempDB].dbo.ExecutionCache AS EC on SN.SnapshotDataID = EC.SnapshotDataID
           INNER JOIN [aapssrs01_ReportServerTempDB].dbo.TempCatalog AS C ON EC.ReportID = C.TempCatalogID
        WHERE
           ContextPath = @Path
        ...
        DELETE
        FROM
           [aapssrs01_ReportServerTempDB].dbo.TempCatalog
        WHERE
            ContextPath = @Path
