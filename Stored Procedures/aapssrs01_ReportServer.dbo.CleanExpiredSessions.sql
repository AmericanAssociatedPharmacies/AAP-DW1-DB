...
/****** Object: SqlProcedure [dbo].[CleanExpiredSessions] Script Date: 12/21/2020 8:32:56 AM ******/
SET ANSI_NULLS ON
GO
...
FROM [aapssrs01_ReportServerTempDB].dbo.SessionData
WHERE 0 = 1;
...
begin
  ...
  from [aapssrs01_ReportServerTempDB].dbo.SessionLock sl with(rowlock, readpast)
  join [aapssrs01_ReportServerTempDB].dbo.SessionData s with(readpast) on sl.SessionID = s.SessionID
  ...
  delete top(20) l
  from [aapssrs01_ReportServerTempDB].dbo.SessionData l
...
begin
  ...
  from [aapssrs01_ReportServerTempDB].dbo.SessionData s with(readpast)
  left join [aapssrs01_ReportServerTempDB].dbo.SessionLock sl with(nolock) on sl.SessionID = s.SessionID
...
delete top(20) ps
from [aapssrs01_ReportServerTempDB].dbo.PersistedStream as ps
