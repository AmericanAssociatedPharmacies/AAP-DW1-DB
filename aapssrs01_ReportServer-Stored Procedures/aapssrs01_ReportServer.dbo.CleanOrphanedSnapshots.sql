...
/****** Object: SqlProcedure [dbo].[CleanOrphanedSnapshots] Script Date: 12/21/2020 8:33:48 AM ******/
SET ANSI_NULLS ON
GO
...
AS
    ...
    begin
        ...
        and not exists (
            select 1 from [aapssrs01_ReportServerTempDB].dbo.SegmentedChunk TSC with(nolock)
    ...
    begin transaction
    DELETE TOP (@TemporarySnapshotCount) [aapssrs01_ReportServerTempDB].dbo.SnapshotData
    output deleted.SnapshotDataID into @cleanedSnapshots(SnapshotDataId)
    FROM [aapssrs01_ReportServerTempDB].dbo.SnapshotData with(readpast)
    WHERE   [aapssrs01_ReportServerTempDB].dbo.SnapshotData.PermanentRefCount <= 0 AND
            [aapssrs01_ReportServerTempDB].dbo.SnapshotData.TransientRefCount <= 0 AND
            [aapssrs01_ReportServerTempDB].dbo.SnapshotData.Machine = @Machine ;
    SET @SnapshotsCleaned = @SnapshotsCleaned + @@ROWCOUNT ;

    DELETE [aapssrs01_ReportServerTempDB].dbo.ChunkData FROM [aapssrs01_ReportServerTempDB].dbo.ChunkData with (readpast)
    INNER JOIN @cleanedSnapshots cs
    ON [aapssrs01_ReportServerTempDB].dbo.ChunkData.SnapshotDataID = cs.SnapshotDataId
    ...
    begin
        delete SC
        output deleted.ChunkId into @cleanedChunks(ChunkId)
        from [aapssrs01_ReportServerTempDB].dbo.SegmentedChunk SC with (readpast)
        join @cleanedSnapshots cs on SC.SnapshotDataId = cs.SnapshotDataId ;
        set @deleteCount = @@ROWCOUNT ;
        set @ChunksCleaned =  @ChunksCleaned + @deleteCount ;
    end ;
    ...
    begin
        ...
        output deleted.ChunkId, deleted.SegmentId into @cleanedSegments (ChunkId, SegmentId)
        from [aapssrs01_ReportServerTempDB].dbo.ChunkSegmentMapping CSM with (readpast)
        join @cleanedChunks cc ON CSM.ChunkId = cc.ChunkId
        where not exists (
            select 1 from [aapssrs01_ReportServerTempDB].dbo.SegmentedChunk SC with(nolock)
            where SC.ChunkId = cc.ChunkId ) ;
        set @deleteCount = @@ROWCOUNT ;
        set @MappingsCleaned = @MappingsCleaned + @deleteCount ;
    end ;
    ...
    begin
        ...
        from [aapssrs01_ReportServerTempDB].dbo.Segment S with (readpast)
        join @cleanedSegments cs on S.SegmentId = cs.SegmentId
        where not exists (
